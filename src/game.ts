// Neon Snake — logique du jeu
// Grille classique, bords en mode "traversée" (téléportation), vitesse progressive,
...
const COLS = 25;
const ROWS = 25;
const CELL = 20; // px, canvas = 500x500
const BEST_SCORE_KEY = "neon-snake-best-score";

const START_SPEED_MS = 150;
const MIN_SPEED_MS = 65;
const SPEED_STEP_MS = 4; // réduction de l'intervalle à chaque fruit mangé
...
class SnakeGame {
  ...
  private overlayMessageEl: HTMLElement;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.overlayButton = this.requireEl("overlay-button") as HTMLButtonElement;

    this.bestScore = Number(localStorage.getItem(BEST_SCORE_KEY) ?? "0") || 0;
  ...
  private queueDirection(dir: Direction): void {
    ...
    if (OPPOSITE[dir] === this.direction) return; // interdit le demi-tour direct
  ...
  private showOverlay(title: string, message: string, buttonLabel: string): void {
  ...
  private handleOverlayAction(): void {
    ...
    // (re)démarrage, que ce soit le tout premier lancement ou après un game over
    this.reset();
    this.hideOverlay();
    this.running = true;
    this.scheduleNextTick();
  ...
  private update(): void {
    ...
    // traversée des bords : téléportation de l'autre côté
  ...
  private render(): void {
    ...
    // nourriture (pulsation néon magenta, désactivée si l'utilisateur préfère moins de mouvement)
    ...
    // serpent (dégradé cyan -> violet, tête plus lumineuse)
